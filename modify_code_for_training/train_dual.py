...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {
            'lr0': (1, 1e-3, 5e-2),           # 초기 학습률 (보통 1e-2 근처가 잘 맞음)
            'lrf': (1, 0.1, 0.5),             # OneCycle 비율. 작을수록 천천히 줄어듦
            'momentum': (1, 0.7, 0.98),       # 관성 계수. 너무 낮으면 진동
            'weight_decay': (1, 0.0, 0.001),   # 정규화. 작은 데이터에서는 과도하게 크면 위험
            'warmup_epochs': 3.0,
            'warmup_momentum': 0.8,
            'warmup_bias_lr': 0.1,
            'box': 7.5,
            ...
            'iou_t': 0.20,
            'anchor_t': 5.0,
            #'anchors': 5.0,  # anchors per output layer (optional)
            'fl_gamma': 0.0,
            'hsv_h': 0.015,
            'hsv_s': 0.7,
            'hsv_v': 0.4,
        ...
        for _ in range(opt.evolve):  # generations to evolve
            if evolve_csv.exists():  # if evolve.csv exists: select best hyps and mutate
                ...
                g = np.array([
                    meta[k][0] if (k in meta and isinstance(meta[k], tuple))
                    else meta[k] if k in meta
                    else 0.0
                    for k in hyp.keys()
                ])
                ng = len(g)
